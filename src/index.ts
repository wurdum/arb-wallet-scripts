...
async function balanceCommand(args: string[]) {
  ...
  if (args.length >= 1 && ethers.isAddress(args[0])) {
...
async function transferCommand(args: string[]) {
  ...
  try {
    ...
    const txFee = gasPrice * estimatedGas;
    console.log(
      `Estimated transaction fee: ${ethers.formatEther(txFee)} WUETH`,
    );

    // Check if we have enough balance
    const totalNeeded = amountWei + txFee;
    if (ethers.parseEther(sourceBalanceBefore) < totalNeeded) {
      console.error("Error: Insufficient funds for transfer + gas");
      console.log(`Available: ${sourceBalanceBefore} WUETH`);
      console.log(
        `Required: ${ethers.formatEther(totalNeeded)} WUETH (${amount} + ${ethers.formatEther(txFee)} gas)`,
      );
      process.exit(1);
    }
